...
function deleteEntity(store, args) {
  ...
  if (eMap) {
    for (const a of eMap.keys()) {
      ...
      if (attr && attr["on-delete-reverse"] === "cascade") {
        allMapValues(eMap.get(a), 1).forEach(([e, a2, v]) => {
          var _a2;
          return deleteEntity(store, [v, (_a2 = attr["reverse-identity"]) === null || _a2 === void 0 ? void 0 : _a2[1]]);
        });
      }
      if (
  ...
  if (vaeTriples) {
    vaeTriples.forEach((triple2) => {
      ...
      if (attr && attr["on-delete"] === "cascade" && ((_b2 = attr["reverse-identity"]) === null || _b2 === void 0 ? void 0 : _b2[1]) === etype) {
...
function transact(store, txSteps) {
  const txStepsFiltered = txSteps.filter(([action, eid, attrId, value, opts]) => {
    ...
    if (attr) {
      const idAttr2 = getPrimaryKeyAttr(store, attr["forward-identity"][1]);
      exists = !!findTriple(store, [eid, idAttr2.id, eid]);
    }
...
// node_modules/@instantdb/core/dist/esm/utils/uuid.js
...
function objectAttrs(store, etype, dq) {
  ...
  if (!Array.isArray(dq.fields)) {
    return getBlobAttrs(store, etype);
  }
  const attrs = /* @__PURE__ */ new Map();
  for (const field of dq.fields) {
    const attr = getAttrByFwdIdentName(store, etype, field);
  ...
  if (!attrs.has("id")) {
    const attr = getAttrByFwdIdentName(store, etype, "id");
...
function runDataloadAndReturnObjects(store, { etype, pageInfo, dq, form }) {
  ...
  if (orderAttr && ((_b = orderAttr === null || orderAttr === void 0 ? void 0 : orderAttr["forward-identity"]) === null || _b === void 0 ? void 0 : _b[2]) !== "id") {
    ...
    idVecs = idVecs.map(([id2]) => {
      ...
      let v = (_e = (_d = (_c = (_b2 = (_a2 = store.eav.get(id2)) === null || _a2 === void 0 ? void 0 : _a2.get(a)) === null || _b2 === void 0 ? void 0 : _b2.values()) === null || _c === void 0 ? void 0 : _c.next()) === null || _d === void 0 ? void 0 : _d.value) === null || _e === void 0 ? void 0 : _e[2];
      if (isDate) {
        v = comparableDate(v);
      }
      return [id2, v];
  ...
  idVecs.sort(direction === "asc" ? function compareIdVecs(a, b) {
    return compareOrder(a, b, orderAttr === null || orderAttr === void 0 ? void 0 : orderAttr["checked-data-type"]);
  } : function compareIdVecs(a, b) {
    return compareOrder(b, a, orderAttr === null || orderAttr === void 0 ? void 0 : orderAttr["checked-data-type"]);
  });
  let objects = {};
...
function convertOpts({ stores, attrs }, [etype, eid, obj_, opts]) {
  return (opts === null || opts === void 0 ? void 0 : opts.upsert) === false ? { mode: "update" } : (opts === null || opts === void 0 ? void 0 : opts.upsert) === true ? null : checkEntityExists(stores, etype, eid) ? { mode: "update" } : null;
}
...
function createMissingAttrs({ attrs: existingAttrs, schema: schema2 }, ops) {
  ...
  for (const op of ops) {
    ...
    if (OBJ_ACTIONS.has(action)) {
      const idAttr2 = getAttrByFwdIdentName2(attrs, etype, "id");
      addUnsynced(idAttr2);
      if (!idAttr2) {
...
function verifyMagicCode(_a) {
  return __awaiter4(this, arguments, void 0, function* ({ apiURI, appId, email, code, refreshToken }) {
...
function signInAsGuest(_a) {
  return __awaiter4(this, arguments, void 0, function* ({ apiURI, appId }) {
    const res = yield jsonFetch(`${apiURI}/runtime/auth/sign_in_guest`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        "app-id": appId
...
function exchangeCodeForToken(_a) {
  return __awaiter4(this, arguments, void 0, function* ({ apiURI, appId, code, codeVerifier, refreshToken }) {
...
function uploadFile(_a) {
  return __awaiter5(this, arguments, void 0, function* ({ apiURI, appId, path, file, refreshToken, contentType, contentDisposition }) {
    const headers = {
      app_id: appId,
      path,
...
var PersistedObject = class {
  constructor(persister, key, defaultValue, onMerge, serialize = (x) => {
    ...
    return x;
  }, saveThrottleMs = 100, idleCallbackMaxWaitMs = 1e3) {
    this._subs = [];
    this._nextSave = null;
...
var DataAttrDef = class _DataAttrDef {
  ...
  unique() {
    return new _DataAttrDef(this.valueType, this.required, this.isIndexed, Object.assign(Object.assign({}, this.config), { unique: true }));
  }
  indexed() {
    return new _DataAttrDef(this.valueType, this.required, true, Object.assign(Object.assign({}, this.config), { indexed: true }));
  }
};
  ...
  for (const key of Object.keys(queryPart)) {
    if (key !== "$") {
      ...
      if (typeof nestedQuery === "object" && nestedQuery !== null) {
        const linkedEntityName = (_a = schema2 === null || schema2 === void 0 ? void 0 : schema2.entities[entityName].links[key]) === null || _a === void 0 ? void 0 : _a.entityName;
        if (linkedEntityName) {
...
function querySubsFromStorage(x, useDateObjects) {
  ...
  for (const key in parsed) {
    ...
    if ((_a = v === null || v === void 0 ? void 0 : v.result) === null || _a === void 0 ? void 0 : _a.store) {
      ...
      v.result.store = fromJSON(Object.assign(Object.assign({}, storeJSON), { useDateObjects }));
    }
  }
  return parsed;
}
...
var Reactor = class {
  constructor(config, Storage2 = IndexedDBStorage, NetworkListener = WindowNetworkListener, versions, EventSourceConstructor) {
    ...
    this._onMergeQuerySubs = (_storageSubs, inMemorySubs) => {
      ...
      Object.entries(inMemorySubs).forEach(([hash, querySub]) => {
        var _a2;
        const storageResult = (_a2 = storageSubs === null || storageSubs === void 0 ? void 0 : storageSubs[hash]) === null || _a2 === void 0 ? void 0 : _a2.result;
        const memoryResult = querySub.result;
        if (storageResult && !memoryResult) {
          ret[hash].result = storageResult;
        }
      });
      const storageKsToAdd = Object.keys(storageSubs).filter((k) => !inMemorySubs[k]).sort((a, b) => {
        var _a2, _b2;
        const aTime = ((_a2 = storageSubs[a]) === null || _a2 === void 0 ? void 0 : _a2.lastAccessed) || 0;
        const bTime = ((_b2 = storageSubs[b]) === null || _b2 === void 0 ? void 0 : _b2.lastAccessed) || 0;
        return bTime - aTime;
    ...
    this.notifyOne = (hash) => {
      var _a2, _b2;
      const cbs = (_a2 = this.queryCbs[hash]) !== null && _a2 !== void 0 ? _a2 : [];
      const prevData = (_b2 = this._dataForQueryCache[hash]) === null || _b2 === void 0 ? void 0 : _b2.data;
    ...
    this.notifyOneQueryOnce = (hash) => {
      var _a2;
      const dfds = (_a2 = this.queryOnceDfds[hash]) !== null && _a2 !== void 0 ? _a2 : [];
      const data = this.dataForQuery(hash);
      dfds.forEach((r) => {
      ...
      try {
        const txSteps = transform({
          ...
          stores: Object.values(this.querySubs.currentValue).map((sub) => {
            var _a2;
            return (_a2 = sub === null || sub === void 0 ? void 0 : sub.result) === null || _a2 === void 0 ? void 0 : _a2.store;
          }),
    ...
    this._transportOnOpen = (e) => {
      ...
      this.getCurrentUser().then((resp) => {
        ...
        this._trySend(uuid_default(), {
          ...
          "refresh-token": (_a2 = resp.user) === null || _a2 === void 0 ? void 0 : _a2["refresh_token"],
    ...
    this._log = createLogger(config.verbose || devBackend || instantLogs);
    ...
    if (typeof BroadcastChannel === "function") {
      ...
      this._broadcastChannel.addEventListener("message", (e) => __awaiter8(this, void 0, void 0, function* () {
        var _a2;
        try {
          if (((_a2 = e.data) === null || _a2 === void 0 ? void 0 : _a2.type) === "auth") {
  ...
  updateSchema(schema2) {
    this.config = Object.assign(Object.assign({}, this.config), { schema: schema2, cardinalityInference: Boolean(schema2) });
  ...
  _initStorage(Storage2) {
    ...
    this.querySubs = new PersistedObject(this._persister, "querySubs", {}, this._onMergeQuerySubs, querySubsToStorage, (x) => querySubsFromStorage(x, this.config.useDateObjects));
  ...
  _finishTransaction(status, eventId, errorMsg) {
    ...
    if (ok) {
      ...
      if (errorMsg.type) {
        const { status: status2 } = errorMsg, body = __rest(errorMsg, ["status"]);
  ...
  _handleReceive(connId, msg) {
    ...
    switch (msg.op) {
      ...
      case "add-query-ok":
        ...
        const store = createStore(this.attrs, triples, enableCardinalityInference, this._linkIndex, this.config.useDateObjects);
      ...
      case "refresh-ok":
        ...
        const updates = computations.map((x) => {
          ...
          const store2 = createStore(this.attrs, triples2, enableCardinalityInference, this._linkIndex, this.config.useDateObjects);
          const newStore = this._applyOptimisticUpdates(store2, mutations, processedTxId);
          ...
          return { hash: hash2, store: newStore, pageInfo: pageInfo2, aggregate: aggregate2 };
        });
        updates.forEach(({ hash: hash2, store: store2, pageInfo: pageInfo2, aggregate: aggregate2 }) => {
          this.querySubs.set((prev) => {
            prev[hash2].result = { store: store2, pageInfo: pageInfo2, aggregate: aggregate2, processedTxId };
            return prev;
          });
        });
        this._cleanupPendingMutationsQueries();
  ...
  _applyOptimisticUpdates(store, mutations, processedTxId) {
    for (const [_, mut] of mutations) {
      if (!mut["tx-id"] || processedTxId && mut["tx-id"] > processedTxId) {
        store = transact(store, mut["tx-steps"]);
      }
    }
    return store;
  }
  ...
  signInWithMagicCode(_a) {
    return __awaiter8(this, arguments, void 0, function* ({ email, code }) {
      var _b;
  ...
  exchangeCodeForToken(_a) {
    return __awaiter8(this, arguments, void 0, function* ({ code, codeVerifier }) {
      var _b;
      const currentUser = yield this.getCurrentUser();
      const isGuest = ((_b = currentUser === null || currentUser === void 0 ? void 0 : currentUser.user) === null || _b === void 0 ? void 0 : _b.type) === "guest";
  ...
  signInWithIdToken(_a) {
    return __awaiter8(this, arguments, void 0, function* ({ idToken, clientName, nonce }) {
      var _b;
      const currentUser = yield this.getCurrentUser();
  ...
  _tryJoinRoom(roomId, data) {
    this._trySendAuthed(uuid_default(), { op: "join-room", "room-id": roomId, data });
  ...
  uploadFile(path, file, opts) {
    return __awaiter8(this, void 0, void 0, function* () {
      ...
      const currentUser = yield this.getCurrentUser();
      const refreshToken = (_a = currentUser === null || currentUser === void 0 ? void 0 : currentUser.user) === null || _a === void 0 ? void 0 : _a.refresh_token;
      return uploadFile(Object.assign(Object.assign({}, opts), { apiURI: this.config.apiURI, appId: this.config.appId, path, file, refreshToken }));
    });
  }
  deleteFile(path) {
    return __awaiter8(this, void 0, void 0, function* () {
...
function schema({ entities, links, rooms: rooms2 }) {
  ...
  const roomsDef = rooms2 !== null && rooms2 !== void 0 ? rooms2 : {};
  return new InstantSchemaDef(
...
function initSchemaHashStore() {
  var _a;
  globalThis.__instantDbSchemaHashStore = (_a = globalThis.__instantDbSchemaHashStore) !== null && _a !== void 0 ? _a : /* @__PURE__ */ new WeakMap();
...
function schemaHash(schema2) {
  ...
  if (schemaHashStore.get(schema2)) {
    return schemaHashStore.get(schema2);
  }
...
function init(config, Storage2, NetworkListener, versions, EventSourceImpl) {
  ...
  const reactor = new Reactor(Object.assign(Object.assign(Object.assign({}, defaultConfig2), config), { cardinalityInference: config.schema ? true : false }), Storage2 || IndexedDBStorage, NetworkListener || WindowNetworkListener, Object.assign(Object.assign({}, versions || {}), { "@instantdb/core": version_default2 }), EventSourceImpl);
...
// node_modules/@instantdb/react-common/dist/esm/InstantReactAbstractDatabase.js
var import_jsx_runtime = __toESM(require_jsx_runtime(), 1);
var import_react4 = __toESM(require_react(), 1);

// node_modules/@instantdb/react-common/dist/esm/useQuery.js
...
function useQueryInternal(_core, _query, _opts) {
  ...
  const subscribe = (0, import_react.useCallback)(
    (cb) => {
      resultCacheRef.current = stateForResult(_core._reactor.getPreviousResult(query3));
      if (!query3) {
        const unsubscribe2 = () => {
        };
        return unsubscribe2;
...
// node_modules/@instantdb/react-common/dist/esm/InstantReactRoom.js
var import_react3 = __toESM(require_react(), 1);

// node_modules/@instantdb/react-common/dist/esm/useTimeout.js
...
function usePublishTopic(room, topic) {
  (0, import_react3.useEffect)(() => room.core._reactor.joinRoom(room.id), [room.id]);
...
function usePresence(room, opts = {}) {
  ...
  const [state, setState] = (0, import_react3.useState)(() => {
    var _a2;
    return (_a2 = room.core._reactor.getPresence(room.type, room.id, opts)) !== null && _a2 !== void 0 ? _a2 : {
      peers: {},
      isLoading: true
    };
  });
...
function useTypingIndicator(room, inputName, opts = {}) {
  ...
  const setActive = (0, import_react3.useCallback)((isActive) => {
    var _a;
...
var InstantReactAbstractDatabase = class {
  constructor(config, versions) {
    ...
    this.useLocalId = (name) => {
      const [localId, setLocalId] = (0, import_react4.useState)(null);
      (0, import_react4.useEffect)(() => {
        let mounted = true;
        const f2 = () => __awaiter9(this, void 0, void 0, function* () {
          const id2 = yield this.getLocalId(name);
          if (!mounted)
            return;
          setLocalId(id2);
        });
    ...
    this.useQuery = (query3, opts) => {
      return useQueryInternal(this.core, query3, opts).state;
    };
    this.useAuth = () => {
      const resultCacheRef = (0, import_react4.useRef)(this.core._reactor._currentUserCached);
      const subscribe = (0, import_react4.useCallback)((cb) => {
      ...
      const state = (0, import_react4.useSyncExternalStore)(subscribe, () => resultCacheRef.current, () => defaultAuthState);
      return state;
    };
    this.useUser = () => {
    ...
    this.useConnectionStatus = () => {
      const statusRef = (0, import_react4.useRef)(this.core._reactor.status);
      const subscribe = (0, import_react4.useCallback)((cb) => {
    ...
    this.queryOnce = (query3, opts) => {
      return this.core.queryOnce(query3, opts);
    };
  ...
  room(type = "_defaultRoomType", id2 = "_defaultRoomId") {
    return new InstantReactRoom(this.core, type, id2);
  }
  /**
   * One time query for the logged in state. This is useful
   * for scenarios where you want to know the current auth
...
getRequestOptions_fn = function() {
  ...
  return "window" in globalThis && (init3.credentials = this.withCredentials ? "include" : "same-origin"), init3;
...
// node_modules/@instantdb/react/dist/esm/InstantReactWebDatabase.js
...
var InstantReactWebDatabase_default = InstantReactWebDatabase;

// node_modules/@instantdb/react/dist/esm/version.js
var version_default3 = version;

// node_modules/@instantdb/react/dist/esm/init.js
function init2(config) {
  return new InstantReactWebDatabase_default(config, {
    "@instantdb/react": version_default3
  });
}
var init_experimental = init2;

// node_modules/@instantdb/react/dist/esm/Cursors.js
...
function Cursors({ as = "div", spaceId: _spaceId, room, className, style, userCursorColor, children, renderCursor, propagate, zIndex }) {
  ...
  return (0, import_react5.createElement)(as, {
    onMouseMove,
    onMouseOut,
    onTouchMove,
    onTouchEnd,
    ...
    children,
    (0, import_jsx_runtime2.jsx)("div", { style: Object.assign(Object.assign(Object.assign({}, absStyles), inertStyles), { zIndex: zIndex !== void 0 ? zIndex : defaultZ }), children: Object.entries(cursorsPresence.peers).map(([id2, presence]) => {
      const cursor = presence[spaceId];
      if (!cursor)
        return null;
      return (0, import_jsx_runtime2.jsx)("div", { style: Object.assign(Object.assign({}, absStyles), { transform: `translate(${cursor.xPercent}%, ${cursor.yPercent}%)`, transformOrigin: "0 0", transition: "transform 100ms" }), children: renderCursor ? renderCursor({
        color: cursor.color,
        presence: fullPresence.peers[id2]
      }) : (0, import_jsx_runtime2.jsx)(Cursor, Object.assign({}, cursor)) }, id2);
    }) }, spaceId)
  ]);
}
...
export {
  ...
  InstantReactWebDatabase_default as InstantReactWebDatabase,
  i,
  uuid_default as id,
  init2 as init,
  init_experimental,
